...
async fn test_snapshot_trigger_and_recovery() {
    ...
    assert!(
        fs::metadata(&snapshot_path).is_ok(),
    ...
    assert!(
        snapshot.last_included_index >= 10,
        "Snapshot should capture at least 10 entries"
    );
    ...
    // --- Part 3: Recovery (Restart) ---
    {
        ...
        assert_eq!(
            result.unwrap(),
            "v0",
            "Value k0 should be recovered from snapshot"
        );
        ...
        assert_eq!(
            result.unwrap(),
            "v14",
            "Value k14 should be recovered from Log"
        );
